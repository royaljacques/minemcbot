...
export default class Ping extends BaseCommand {
  ...
  public async execute(command: ChatInputCommandInteraction) : Promise<void> {
    console.log("info")
    const user = Index.instance.getProfils().getUser(command.user.id);
    if(user === null || user === undefined){
      command.reply({content: "you don't have use /start command", ephemeral: true})
    }else{
      ...
      .setTitle(user === null ? getLanguage("info_embed_title", "en"): getLanguage("info_embed_title", user.language))
      .addFields(
